...
// feature across all AlphaFi pools. Transactions are built server-side by
// alphafi-api (via sdk.autocompound) and dry-run against mainnet.
//
...
//
// Usage:
//   npx tsx scripts/autocompoundScript.ts
//
// Requires env vars: PK_B64, NETWORK
// Optional: ALPHAFI_API_BASE_URL (e.g. http://localhost:8090; default: staging)
//
...
const API_BASE_URL = process.env.ALPHAFI_API_BASE_URL || 'https://api-staging.alphafi.xyz';
...
const context = new StrategyContext('mainnet', undefined, API_BASE_URL);
...
async function fetchPoolConfigs(): Promise<ApiConfig> {
  try {
    const response = await fetch(`${context.apiBaseUrl}/public/config`);
    if (!response.ok) {
...
async function testPoolAutocompound(
  ...
  try {
    ...
    // Create autocompound transaction (server-built; gas budget already set)
...
async function testAllPoolsAutocompound() {
  ...
  try {
    // Initialize SDK
    const sdk = new AlphaFiSDK({ network: 'mainnet', apiBaseUrl: API_BASE_URL });
 ...
 */
async function _testSpecificPools(poolIds: string[]) {
