...
class DepositTester {
  ...
  async testBluefinDeposit(): Promise<boolean> {
    ...
    try {
      ...
      this.log(`Creating deposit transaction for pool ${this.config.bluefinPoolId} with amount ${this.config.depositAmounts.sui}`);
      ...
      return await this.executeTransaction(txb, `Bluefin deposit (Pool ${this.config.bluefinPoolId}, Amount: ${this.config.depositAmounts.sui})`);
    } catch (error) {
  ...
  async testNaviDeposit(): Promise<boolean> {
    ...
    try {
      ...
      return await this.executeTransaction(txb, `Navi deposit (Pool ${this.config.naviPoolId}, Amount: ${this.config.depositAmounts.usdc})`);
    } catch (error) {
  ...
  async testCetusDeposit(): Promise<boolean> {
    ...
    try {
      ...
      return await this.executeTransaction(txb, `Cetus deposit (Pool ${this.config.cetusPoolId}, Amount: ${this.config.depositAmounts.sui})`);
    } catch (error) {
