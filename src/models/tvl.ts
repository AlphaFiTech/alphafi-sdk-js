...
export class TVLManager {
  ...
  async getTVLBreakdown(
    ...
    for (const poolTVL of poolTVLs) {
      ...
      const poolEntry = Object.entries(poolDetailsMap).find(([, details]) => 
        details.poolName === poolTVL.poolName
      );
  ...
  async getPoolGrowthMetrics(
    ...
    const tvlGrowth = previousTVLData.totalValueLockedUSD.isZero() 
      ? new Decimal(0)
      : currentTVL.totalValueLockedUSD.minus(previousTVLData.totalValueLockedUSD)
          .div(previousTVLData.totalValueLockedUSD).mul(100);
