...
export class ClaimRewardsTransactions {
  ...
  async claimAllRewardsTx(): Promise<Transaction> {
    ...
    for (const poolId of poolIds) {
      ...
      if (poolId === '1' || parseInt(poolId.toString()) === 1) {
        continue;
      }
  ...
  async claimRewardsForPool(tx: Transaction, poolId: number, alpha_receipt: any): Promise<any> {
    ...
    const receipts = await this.blockchain.getReceipt(poolId.toString(), this.address);
    ...
    // Convert single receipt to array for backwards compatibility with existing methods
    ...
    //   return this.claimNaviV2Rewards(tx, poolId, receiptsArray, alpha_receipt);
    // } else if (poolinfo.packageId.includes('alphafi_bluefin')) {
    //   return this.claimBluefinRewards(tx, poolId, receiptsArray, alpha_receipt, poolName);
    // } else if (poolinfo.packageId.includes('alphafi_navi')) {
    //   return this.claimNaviRewards(tx, poolId, receiptsArray, alpha_receipt, poolName);
    // } else if (poolinfo.packageId.includes('alphafi_cetus')) {
    //   return this.claimCetusRewards(tx, poolId, receiptsArray, alpha_receipt, poolName);
    // } else if (poolinfo.packageId.includes('alphafi_bucket')) {
    //   return this.claimBucketRewards(tx, poolId, receiptsArray, alpha_receipt);
    // } else {
