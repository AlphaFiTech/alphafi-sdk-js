...
describe('TVLManager', () => {
    ...
    test('should return comprehensive TVL summary', async () => {
      ...
      if (summary.topPoolsByTVL.length > 1) {
        for (let i = 0; i < summary.topPoolsByTVL.length - 1; i++) {
          expect(summary.topPoolsByTVL[i].totalValueLockedUSD.toNumber()).toBeGreaterThanOrEqual(
            summary.topPoolsByTVL[i + 1].totalValueLockedUSD.toNumber(),
          );
        }
      }
    });
    ...
    test('should return top pools by TVL', async () => {
      ...
      if (topPools.length > 1) {
        for (let i = 0; i < topPools.length - 1; i++) {
          expect(topPools[i].totalValueLockedUSD.toNumber()).toBeGreaterThanOrEqual(
            topPools[i + 1].totalValueLockedUSD.toNumber(),
          );
        }
      }
    });
  ...
  describe('mock data validation', () => {
    test('should return different TVLs for different pool types', async () => {
      ...
      expect(alphaTVL.totalValueLockedUSD.toNumber()).not.toEqual(
        naviTVL.totalValueLockedUSD.toNumber(),
      );
      expect(naviTVL.totalValueLockedUSD.toNumber()).not.toEqual(
        cetusTVL.totalValueLockedUSD.toNumber(),
      );
    });
    ...
    test('should maintain consistency between total and USD values', async () => {
      ...
      expect(tvlData.totalValueLockedUSD.toNumber()).toBeCloseTo(calculatedUSD.toNumber(), 2);
    });
  });
